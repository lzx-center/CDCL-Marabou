...
//                    printf("%s\n",Stringf("x%u: upper bound set to %.3lf", variable, bound._value).ascii());
    ...
    printf("Ave length : [%lf] -> [%lf]\n", 1.0 * oTotal / preSearchPath._paths.size(), 1.0 * length / verified);
}

bool Engine::conflictClauseLearning(std::vector<PathElement> &path, std::vector<double> lowerBounds,
    ...
    auto getConstraintByPosition = [&](Position position) {
        if (position._layer) {
            return positionToConstraint.at(position);
        } else {
            return getDisjunctionConstraintBasedOnIntervalWidth(position._node);
        }
    };
